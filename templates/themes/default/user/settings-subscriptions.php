...
<section class="panel">
    <header class="panel-head">
        <h2 class="panel-title">Subscribed topics</h2>
        <span class="panel-meta mono"><?= $this->number($paginator->total()) ?> topic(s)</span>
    </header>
<?php if ($paginator->items() === []): ?>
    <?= $this->partial('partials/empty', ['message' => 'You are not following any topic yet. Use “Subscribe” inside a topic.']) ?>
<?php else: ?>
    <table class="board-table topic-table">
        <caption class="visually-hidden">Topics you follow</caption>
        <thead>
            <tr>
